...
import type { Project, Task, FrictionLevel, ProjectStatus, BackupData } from '../types';
  ...
  const [projects, setProjects] = useState<Project[]>(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        ...
        } catch { console.error('Failed to parse projects'); }
      }
    }
    return [];
  });
  ...
  const [tasks, setTasks] = useState<Task[]>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        try {
          return JSON.parse(saved);
        } catch { console.error('Failed to parse tasks'); }
      }
    }
    return [];
  });
